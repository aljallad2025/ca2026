...
<ion-content [fullscreen]="true" class="bookings-content">
  ...
  <ion-segment [(ngModel)]="activeTab" (ionChange)="loadBookings()" mode="md" class="status-segment">
    ...
    <ion-segment-button value="pending"><ion-label>Upcoming</ion-label></ion-segment-button>
  ...
  <div class="bookings-list" *ngIf="!loading">
    <div class="booking-card" *ngFor="let b of bookings">
      ...
      <div class="booking-footer">
        <span class="price">{{ b.total_amount | number }} <small>SAR</small></span>
        <div class="btns">
          <button *ngIf="b.status === 'active'" class="action-btn primary" (click)="extend(b)">Extend</button>
          <button *ngIf="b.status !== 'cancelled' && b.status !== 'completed'" class="action-btn danger" (click)="cancel(b)">Cancel</button>
        </div>
      </div>
    </div>
...
export class MyBookingsPage implements OnInit {
  ...
  statusLabel(status: string) {
    const map: any = { active: 'Active', pending: 'Upcoming', completed: 'Completed', cancelled: 'Cancelled' };
