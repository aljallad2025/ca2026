...
import { personCircleOutline, createOutline, documentTextOutline, locationOutline, cardOutline, chatbubbleEllipsesOutline, heartOutline, moonOutline, notificationsOutline, chevronForwardOutline, logOutOutline, languageOutline } from 'ionicons/icons';

@Component({
  selector: 'app-profile',
  template: `
<ion-content [fullscreen]="true" class="profile-content">
  <div class="page-header"><h1>{{ 'profile.myAccount' | t }}</h1></div>

  <div class="profile-card">
    <div class="avatar"><ion-icon name="person-circle-outline"></ion-icon></div>
    <h2>{{ user?.full_name || ('home.user' | t) }}</h2>
    <p>{{ user?.email }}</p>
  ...
  <div class="menu-section">
    <button class="menu-item" *ngFor="let item of menuItems" (click)="item.action()">
      ...
      <span class="menu-label">{{ item.labelKey | t }}</span>
      <ion-icon name="chevron-forward-outline" class="menu-arrow"></ion-icon>
    </button>
  </div>

  <div class="toggle-section">
    <div class="toggle-item lang-row">
      <span><ion-icon name="language-outline"></ion-icon>{{ 'profile.language' | t }}</span>
      <div class="lang-switch">
        <button [class.active]="lang.lang()==='en'" (click)="setLang('en')">EN</button>
        <button [class.active]="lang.lang()==='ar'" (click)="setLang('ar')">AR</button>
      </div>
    </div>
    <div class="toggle-item">
      <span><ion-icon name="moon-outline"></ion-icon>{{ 'profile.darkMode' | t }}</span>
      <ion-toggle [(ngModel)]="darkMode" (ionChange)="toggleDarkMode()"></ion-toggle>
    </div>
    <div class="toggle-item">
      <span><ion-icon name="notifications-outline"></ion-icon>{{ 'profile.notifications' | t }}</span>
  ...
  <div class="version">{{ 'profile.appVersion' | t }} 1.0.0</div>
...
.menu-item {
  ...
  .menu-label { flex: 1; color: #f4f4f5; font-size: 14px; text-align: start; }
...
.lang-switch {
  display: flex; gap: 4px; background: #1c1c1c; border-radius: 10px; padding: 3px;
  button {
...
export class ProfilePage {
  ...
  constructor(public auth: AuthService, public lang: LanguageService, private router: Router) {
    addIcons({ personCircleOutline, createOutline, documentTextOutline, locationOutline, cardOutline, chatbubbleEllipsesOutline, heartOutline, moonOutline, notificationsOutline, chevronForwardOutline, logOutOutline, languageOutline });
  }

  get user() { return this.auth.currentUser(); }

  setLang(l: 'en' | 'ar') { this.lang.setLang(l); }
